...
from ModuleR_patch_self_test2 import *
import torch
...
class GPTReDo(pl.LightningModule): 
    ...
    def __init__(self, config_GPT, config_others):
        ...
        self.token_self = nn.ModuleDict(dict(
            h = nn.ModuleList([SelfAttnTokenLevelBlock(config_GPT) for _ in range(config_GPT.n_layer)]),
            ln_f = LayerNorm(config_GPT.n_embd, bias=config_GPT.bias),
        ))
    ...
    def forward(self, idx1, idx2, targets=None):
        ...
        x_patched = self.transformer.ln_f(x_patched) #bz, sq/pz, dim
        predict = self.up(x_patched) #bz, sq/pz, dim * pz
        # patch to token
        #bz, sq/pz, pz, dim
        ...
        loss_auto = F.cross_entropy(autoencoder_logits.view(-1, autoencoder_logits.size(-1)), idx1.view(-1), ignore_index=-1)
        ...
        for block in self.token_self.h:
            x2 = block(x2.view(b, -1, self.patch_size, self.config_GPT.n_embd), predict)
        ...
        return None, loss, loss_auto
        
        
    def training_step(self, batch, batch_idx):
        #print("entering training step")
        train_data1, train_data2, target = batch[0], batch[1], batch[2]
        #print("training shape", train_data.shape)
        pred_logits, loss, loss_auto = self.forward(train_data1, train_data2, target)
        assert loss != None
    ...
    def validation_step(self, batch, batch_idx) :
        ...
        train_data1, train_data2, target = batch[0], batch[1], batch[2]
        pred_logits, loss , auto_loss= self.forward(train_data1, train_data2, target)
        assert loss != None
        log_args = dict(on_step=False, on_epoch=True, sync_dist=True, add_dataloader_idx=False)
        self.log("valid/CELoss", loss.detach(), **log_args)
        return loss+auto_loss
