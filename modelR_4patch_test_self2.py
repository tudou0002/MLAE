...
class GPTReDo(pl.LightningModule): 
    ...
    def forward(self, idx, targets=None):
        ...
        x_patched = x_encoder.view(b, -1, self.patch_size, self.config_GPT.n_embd)
        #bz, sq/pz, pz * dim
        ...
        loss_auto = F.cross_entropy(autoencoder_logits.view(-1, autoencoder_logits.size(-1)), idx.view(-1)[:b * self.config_GPT.block_size], ignore_index=-1)
        
        x_again = x[:, 15:, :].view(b, -1, self.patch_size, self.config_GPT.n_embd)
        #predict = predict.view(b, -1, self.config_GPT.n_embd)
        
        
        predict = self.token_self(x_again , predict)
        #bz, sq, dim
        ...
        return logits, loss, loss_auto
        
        
    def training_step(self, batch, batch_idx):
        #print("entering training step")
        train_data, target = batch[0], batch[1]
        #print("training shape", train_data.shape)
        pred_logits, loss, loss_auto = self.forward(train_data, target)
        assert loss != None
    ...
    def validation_step(self, batch, batch_idx) :
        ...
        train_data, target = batch[0], batch[1]
        pred_logits, loss , auto_loss= self.forward(train_data, target)
        assert loss != None
        log_args = dict(on_step=False, on_epoch=True, sync_dist=True, add_dataloader_idx=False)
